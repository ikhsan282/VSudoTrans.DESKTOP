...
{
    public partial class frmBrandLV : frmBaseFilterLV
    {
        public frmBrandLV()
        {
            InitializeComponent();
        ...
        {
            string url = "/BrandVehicles/Import/ValidateFile";
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    ImportSummaryBrandVehicleModel result = null;
                    try
                    {
                        result = HelperRestSharp.UploadFileImport<ImportSummaryBrandVehicleModel>(File.ReadAllBytes(openFileDialog.FileName), openFileDialog.FileName, url);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (result != null)
                    {
                        using (var form = new frmImportBrandWV())
                        {
                            ...
                            if (resultDialog == System.Windows.Forms.DialogResult.OK)
                            {
                                ...
                                var response = HelperRestSharp.Post("/BrandVehicles/Import", jsonString);
        ...
        {
            var fileExcel = HelperRestSharp.DownloadFile("vsudotrans", "import/Import Brand.xlsx");
            HelperRestSharp.SaveFileDialog(fileExcel, "File Template Import Brand");
        }
